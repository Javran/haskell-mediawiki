...
mwApiUrl :: String
mwApiUrl = "http://en.wikipedia.org/w/"
...
getAll url pg acc mbCont = do
    ...
    case LL.stringXml i of
        ...
        Right ll -> do
            let lls = map (\lli -> (langName lli, lTitle lli)) (concatMap snd (llPages ll))
            case llContinue ll of
                Nothing -> return $ concat $ reverse (lls:acc)
                Just mb -> getAll url pg (lls:acc) (Just mb)
  where
    lTitle x =
        case langTitle x of
            Nothing -> pg
